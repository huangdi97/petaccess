...
interface Audit {
  id: string; actor_user_id: string | null; actor_role: string | null; action: string;
  target_type: string; target_id: string; before_state: unknown; after_state: unknown;
...
async function load() {
  error.value = "";
  try {
    items.value = (await page<Audit>("/admin/audit", {
      target_type: targetType.value || undefined, limit: 100,
    })).items;
  } catch (e) {
...
<template>
  ...
  <div class="panel">
    <table>
      <thead><tr><th>时间</th><th>操作</th><th>角色</th><th>目标</th><th>变更</th></tr></thead>
      <tbody>
        ...
        <tr v-if="!items.length"><td colspan="5" class="muted">暂无审计记录</td></tr>
      </tbody>
    </table>
  </div>
</template>
