...
interface Place {
  ...
  canonical_address: string | null; lifecycle_status: string;
}
...
const form = ref({ canonical_name: "", place_type: "cafe", canonical_address: "", location_wkt: "" });
...
async function create() {
  ...
  try {
    ...
    form.value = { canonical_name: "", place_type: "cafe", canonical_address: "", location_wkt: "" };
    await load();
  } catch (e) {
      ...
      <div class="row">
        ...
        <div>
          ...
          <select v-model="form.place_type">
            <option value="cafe">cafe</option><option value="restaurant">restaurant</option>
            <option value="mall">mall</option><option value="park">park</option>
        ...
        <div><label>位置 WKT（POINT(lng lat)，可空）</label><input v-model="form.location_wkt" placeholder="POINT(121.47 31.23)" /></div>
      </div>
      <button class="primary" style="margin-top: 10px" :disabled="!form.canonical_name" @click="create">保存</button>
    </div>
  </div>
  <div class="panel">
    <table>
      <thead><tr><th>名称</th><th>类型</th><th>地址</th><th>状态</th><th></th></tr></thead>
      <tbody>
        <tr v-for="p in items" :key="p.id">
          <td>{{ p.canonical_name }}</td>
          <td><span class="tag">{{ p.place_type }}</span></td>
          <td class="muted">{{ p.canonical_address ?? "—" }}</td>
          <td>{{ p.lifecycle_status }}</td>
          <td><RouterLink :to="`/places/${p.id}`">详情 →</RouterLink></td>
        </tr>
        <tr v-if="!items.length"><td colspan="5" class="muted">{{ busy ? "加载中…" : "暂无数据" }}</td></tr>
      </tbody>
    </table>
  </div>
</template>
