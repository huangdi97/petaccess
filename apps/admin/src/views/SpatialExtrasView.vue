...
<template>
  ...
  <div class="panel">
    ...
    <div class="row">
      <div class="field">
        <label>场所 ID</label
        ><input v-model="amenityForm.place_id" class="mono" placeholder="place uuid" />
      </div>
      <div class="field">
      ...
      <div class="field">
        <label>来源 ID</label
        ><input v-model="amenityForm.source_id" class="mono" placeholder="source uuid" />
      </div>
    </div>
    <button
      class="primary"
      style="margin-top: 10px"
      :disabled="busy === 'amenity' || !amenityForm.place_id || !amenityForm.source_id"
      @click="submit('amenity')"
    >
      登记设施
    </button>
  </div>
  ...
  <div class="panel">
    ...
    <div class="row">
      <div class="field">
        <label>场所 ID</label
        ><input v-model="entranceForm.place_id" class="mono" placeholder="place uuid" />
      </div>
      <div class="field">
        <label>分区 ID（可选）</label
        ><input v-model="entranceForm.zone_id" class="mono" placeholder="zone uuid" />
      </div>
      <div class="field">
    ...
    <div class="row">
      <div class="field">
        <label>位置 WKT</label
        ><input
          v-model="entranceForm.location_wkt"
          class="mono"
          placeholder="POINT(116.40 39.90)"
        />
      </div>
      <div class="field">
        <label>通行说明</label
        ><input v-model="entranceForm.access_notes" placeholder="例如：需绕行至北侧" />
      </div>
      <div class="field">
        <label>来源 ID</label
        ><input v-model="entranceForm.source_id" class="mono" placeholder="source uuid" />
      </div>
    </div>
    <button
      class="primary"
      ...
      :disabled="
        busy === 'entrance' ||
        !entranceForm.place_id ||
        !entranceForm.name ||
        !entranceForm.source_id
      "
      @click="submit('entrance')"
    >
      登记出入口
    </button>
    <p class="hint">
      类型含义：GENERAL 通用 / PET_ALLOWED 允许携宠 / PET_PROHIBITED 禁止携宠 / SERVICE_DOG_ONLY
      仅服务犬。
    </p>
  </div>
  ...
  <div class="panel">
    ...
    <div class="row">
      <div class="field">
        <label>场所 ID</label
        ><input v-model="pathForm.place_id" class="mono" placeholder="place uuid" />
      </div>
      <div class="field">
        <label>路径名称</label><input v-model="pathForm.name" placeholder="从东门到宠物区" />
      </div>
      <div class="field">
        <label>起点节点</label><input v-model="pathForm.from_node" placeholder="east_gate" />
      </div>
      <div class="field">
        <label>终点节点</label><input v-model="pathForm.to_node" placeholder="pet_zone" />
      </div>
    </div>
    <div class="row">
      <div class="field">
      ...
      <div class="field">
        <label>时间窗（可选）</label
        ><input v-model="pathForm.time_window" placeholder="09:00-21:00" />
      </div>
      <div class="field">
        <label>来源 ID</label
    ...
    <button
      class="primary"
      style="margin-top: 10px"
      :disabled="busy === 'path' || !pathForm.place_id || !pathForm.name || !pathForm.source_id"
      @click="submit('path')"
    >
      登记通行路径
    </button>
  ...
  <div class="panel">
    ...
    <table class="compact">
      ...
      <tbody>
        ...
        <tr v-if="!amenities.length">
          <td colspan="6" class="muted">暂无设施记录</td>
        </tr>
      </tbody>
    </table>
  </div>
  ...
  <div class="panel">
    ...
    <table class="compact">
      ...
      <tbody>
        <tr v-for="e in entrances" :key="e.id">
          ...
          <td>
            <span
              class="tag"
              :class="
                e.entrance_type === 'PET_ALLOWED'
                  ? 'ok'
                  : e.entrance_type === 'PET_PROHIBITED'
                    ? 'restricted'
                    : 'unknown'
              "
            >
        ...
        <tr v-if="!entrances.length">
          <td colspan="6" class="muted">暂无出入口记录</td>
        </tr>
      </tbody>
    </table>
  </div>
    ...
    <table class="compact">
      ...
      <tbody>
        ...
        <tr v-if="!paths.length">
          <td colspan="7" class="muted">暂无通行路径记录</td>
        </tr>
      </tbody>
    </table>
  </div>
