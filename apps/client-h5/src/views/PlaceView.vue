  ...
  <AppShell>
    ...
    <template v-else>
      ...
      <div v-if="answer" class="answer panel" :class="'s-' + answer.status" data-testid="answer">
        ...
        <div class="notice">
          来源：{{
            rules[0] ? (sourceMap.get(rules[0].source_id)?.issuer ?? "待收录") : "待收录"
          }}
      ...
      <div class="panel" data-testid="zones">
        ...
        <div v-for="z in answer?.zones ?? []" :key="z.zone.id" class="zone-row">
          <span
            >{{ z.zone.name
            }}<span class="tag" v-if="z.zone.floor_ref" style="margin-left: 6px">{{
              z.zone.floor_ref
            }}</span></span
          >
          <span>
        ...
        <div
          v-for="r in rules.filter((r) => !r.zone_id && r.status === 'current')"
          :key="r.id"
          class="zone-row"
        >
        ...
        <div v-for="r in rules.filter((r) => r.status === 'current')" :key="r.id" class="zone-row">
          ...
          <span class="muted">
            ...
            <button
              ...
              @click="quickConfirm(r.id, 'still_valid')"
            >
              仍有效
            </button>
            <button
              style="margin-left: 4px; padding: 4px 8px"
              @click="quickConfirm(r.id, 'changed')"
            >
              已变化
            </button>
            <button
              style="margin-left: 4px; padding: 4px 8px"
              @click="quickConfirm(r.id, 'uncertain')"
            >
              不确定
            </button>
          </span>
      ...
      <div class="panel">
        ...
        <div class="notice">本页展示已收录的规则和历史现场记录，不构成场所实际无动物的保证。</div>
      </div>
