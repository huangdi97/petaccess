"""Project the signed Wave-02 review register into a publisher-consumable register.
...
`docs/expansion/review_decisions_expansion_r1_wave02.json` was emitted by
`scripts/expansion_w02_reports.py` with a **thinner** schema: it carries the
...
Output: ``docs/expansion/review_decisions_expansion_r1_wave02_publishable.json``
"""
...
SIGNED_REGISTER = REPO / "docs" / "expansion" / "review_decisions_expansion_r1_wave02.json"
OUT_REGISTER = REPO / "docs" / "expansion" / "review_decisions_expansion_r1_wave02_publishable.json"
...
def derive_rule_id(row: dict) -> str:
    """A stable, human-facing id for a Wave-02 candidate.

    Wave-02 candidates were ingested without a ``rule_id`` slug (they predate the
    ...
    rather than invented: deterministic, unique, and obviously a Wave-02 row
    (``w02-`` prefix), so it can never collide with a hand-written R2 slug.
    """
    return f"w02-{row['candidate_id'].replace('-', '')[:10]}"
...
def build_exception_plan(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        ...
        # A carve-out belongs in the plan only when a same-layer base actually
        # governs its subject. For Wave-02 豫园 the source text is
        # 「宠物（符合规定的导盲犬除外）」 — ordinary_pet does NOT cover a guide
        # dog (working animal ≠ pet), so modelling the guide-dog row as an
        # exception of the ordinary_pet base would create an inert carve-out
        # the publisher must refuse. The correct, source-faithful projection is
        # a standalone conditional rule (CREATE_ACCESS_RULE with effect
        # conditional), exactly as ADR-025 prescribes: never widen the base,
        # never drop the exemption — publish the exemption on its own scope.
        same_layer_bases = [b for b in bases if b["same_layer"]]
        if not same_layer_bases:
            # Not an exception plan entry: the human decision stands, the row
            # is projected as an ordinary approved rule.
            continue
...
def main() -> int:
    ...
    doc = {
        "_readme": (
            "Projected from docs/expansion/review_decisions_expansion_r1_wave02.json by "
            "scripts/expansion_w02_publish_register.py. Gate fields are derived from the live "
