...
| # | 项 | 现状 | 影响 |
|---|---|---|---|
| 1 | 地图为 MockMap | 无真实地图 SDK | 地图 Tab 不能用于真实导航/定位；产品语义上仍是「列表 + 模拟地图」 |
| 2 | A11y 覆盖不完整 | 14 个 view 中仅 5 个含 `aria-*`（PlaceView 为 0） | 屏幕阅读器在核心页缺少标签 |
| 3 | 无可视化回归基线 | Playwright 16 例全为功能断言，无截图快照 | 视觉回归靠人工 |
